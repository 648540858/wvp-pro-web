        ...
        >
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.dataIndex === 'statusInGroup'">
              <span v-if="record.commonGbCivilCode"> 已添加 </span>
              <span v-if="!record.commonGbCivilCode"> 未添加 </span>
            </template>
          </template>
          <template #title>
...
<script lang="ts" setup>
  ...
  const columns = channelInRegionColumns()
  ...
  const getChannelList = (): void => {
    queryChannelList({
      ...
      inRegion: statusInGroup.value === '' ? null : statusInGroup.value === 'true',
      regionDeviceId: null,
    })
      .then((result) => {
  ...
  const addChannelToRegion = (): void => {
    if (selectedRowKeys.value.length == 0) {
      message.error('未选择通道')
      return
    }
    let regionKey = resourceTreeRef.value.getSelectKey()
    if (!regionKey) {
      message.error('未选择分组')
      return
    }
    console.log('通道：' + selectedRowKeys.value.length)
    console.log('分组：' + regionKey)
    updateToRegion({
      commonGbIds: selectedRowKeys.value,
      commonGbCivilCode: regionKey,
    })
      .then(() => {
        ...
        resourceTreeRef.value.refreshNote(regionKey, channels)
        getChannelList()
